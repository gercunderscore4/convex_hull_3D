...
void ConvexHull3D(float** p_p, unsigned int* lenp_p, unsigned int** l_p, unsigned int* lenl_p, unsigned int* leni_p, int list_type) {
	...
	if (lenp == 1) {
		...
		BuildPoint(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else if (lenp == 2) {

		BuildLine(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else if (LinearCheck(p, lenp) == TRUE) {

		BuildLinear(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else if (lenp == 3) {

		BuildTriangle(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else if (PlanarCheck(p, lenp) == TRUE) {

		BuildPlanar(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else if (lenp == 4) {

		BuildTetrahedron(p, lenp, l_p, lenl_p, leni_p, list_type);

	} else {

		BuildHull(p, lenp, l_p, lenl_p, leni_p, list_type);
